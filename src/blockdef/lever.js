export function lever(noa,options,id,texture,name,offmeshes){
        ...
        updatable: true
			...
			supermesh2.scaling.y=2
			 ...
			 /*finOpts.onCustomMeshCreate= function (supermesh, x, y, z) {//
				 ...
				 function twist(){
					supermesh.rotation.y=rot[1]
					supermesh.rotation.z=rot[2]
					supermesh.position.y+=rot[3][1]
					return;
			...
			finOpts.onLoad= function ( x, y, z) {
				...
				if(name=='lever'){
				mesh2.position.y=y
				mesh2._children[0].rotation.x=Math.PI/8
				}
				if(name=='leveron'){
				mesh2.position.y=y
				mesh2._children[0].rotation.x=-Math.PI/8
