...
var scene=null
var modelname='cow'
export function defineModelCompMinecart(noa2) {
	noa = noa2
scene=noa.rendering.getScene()
	
	
		
		var bustex = new BABYLON.Texture(mod+"models/minecart/minecart.png", scene, false, true, BABYLON.Texture.NEAREST_SAMPLINGMODE);
...
BABYLON.SceneLoader.ImportMesh("", "", mod+"models/minecart/minecart.babylon", scene, function (meshes) { 
...
export  function applyModelMinecart(eid, model, texture, offset, nametag, name, hitbox,randomizer,rando,entityList,uuid,socket,PF) {
		...
		var hitboxMesh = new BABYLON.MeshBuilder.CreateBox('hitbox:car:'+uuid, {
				height: 1,
				width: 0.8,
				depth: 1,
			}, scene)
		...
		noa.entities.removeComponent(eid, noa.entities.names.shadow)
	...
	c.material.diffuseTexture.update()
		 ...
		 /*noa.entities.addComponent(eid, noa.entities.names.health,{
				mesh: builded,
				mob:true,
				walking:false
			})*/
		builded.rotation.y=0
			//setTimeout(function(){

              /* noa.entities.addComponent(eid, noa.entities.names.followsEntity,{
    ...
    callback: onCollideEnto
		...
		function collideEntityo(noa, ownID, otherID){
			if(noa.entities.hasPhysics(ownID)==false){
				return;
			}
