...
var scene=null
var modelname='cow'
export function defineModelCompBoat(noa2) {
	noa = noa2
scene=noa.rendering.getScene()
	
	
		
		var bustex = new BABYLON.Texture(mod+"models/boat/boat.png", scene, false, true, BABYLON.Texture.NEAREST_SAMPLINGMODE);
...
BABYLON.SceneLoader.ImportMesh("", "", mod+"models/boat/boat.babylon", scene, function (meshes) { 
...
export  function applyModelBoat(eid, model, texture, offset, nametag, name, hitbox,randomizer,rando,entityList,uuid,socket,PF) {
		...
		var hitboxMesh = new BABYLON.MeshBuilder.CreateBox('hitbox:boat:'+uuid, {
				height: 0.3,
				width: 1,
				depth: 1.7,
			}, scene)
