...
var scene=null
var modelname='cow'
export function defineModelCompBokkusu(noa2) {
	noa = noa2
scene=noa.rendering.getScene()
	
	
		
		var bustex = new BABYLON.Texture(mod+"models/bokkusu/bokkusu.png", scene, false, true, BABYLON.Texture.NEAREST_SAMPLINGMODE);
...
BABYLON.SceneLoader.ImportMesh("", "", mod+"models/bokkusu/bokkusu.babylon", scene, function (meshes) { 
...
export  function applyModelBokkusu(eid, model, texture, offset, nametag, name, hitbox,randomizer,rando,entityList,uuid,socket,PF) {

		var builded = meshbody.clone('bokkusu')//
		...
		var hitboxMesh = new BABYLON.MeshBuilder.CreateBox('hitbox:bokkusu:'+uuid, {
				height: 1,
				width: 1,
				depth: 1,
			}, scene)
			...
			hitboxMesh.setPositionWithLocalVector(new BABYLON.Vector3(0, 0.7, 0) )
	...
	//noa.entities.addComponent(eid, noa.entities.names.propulsion)
    ...
    callback: onCollideEnto
	  ...
	  /*var body = noa.entities.getPhysicsBody(ownID)
	    var body2 = noa.entities.getPhysicsBody(otherID)
	   body.applyImpulse([body2.velocity[0],body2.velocity[1],body2.velocity[2]]);
