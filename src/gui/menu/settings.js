...
menu.classList.add('fool')
	...
	modnameDiv.classList.add('form-group')

	var modnameInfo = document.createElement('label')
	modnameInfo.id = 'menu_nickname_info'
	modnameInfo.htmlFor = 'menu_modname'
	modnameInfo.innerHTML = 'ModName'
	modnameInfo.style.marginBottom = '5px'
	modnameDiv.appendChild(modnameInfo)
	
	var modnameInput = document.createElement('input')
	modnameInput.id = 'menu_modname'
	modnameInput.classList.add('form-control')
	modnameInput.classList.add('form-control-sm')
	modnameInput.classList.add('col-md-6')

	modnameInput.setAttribute('placeholder', localStorage.getItem('modname') )
	modnameInput.value = localStorage.getItem('modname')

	modnameDiv.appendChild(modnameInput)

	menu.appendChild(modnameDiv)
	...
	var window = createWindow('menu_settins', 'Settings', ['300px', '200px'], menu)

	var old = window.main.onclick

	window.main.onclick = function() {
		old()
		localStorage.setItem('modname', modnameInput.value)
